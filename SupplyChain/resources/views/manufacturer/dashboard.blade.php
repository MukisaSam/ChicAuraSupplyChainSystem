...
<head>
    ...
    <style>
        ...
        .main-content-visible h1,
        .main-content-visible h2,
        .main-content-visible h3,
        .main-content-visible h4,
        .main-content-visible h5,
        .main-content-visible h6 {
            color: #111827;
        }

        .dark .main-content-visible h1,
        .dark .main-content-visible h2,
        .dark .main-content-visible h3,
        .dark .main-content-visible h4,
        .dark .main-content-visible h5,
        .dark .main-content-visible h6 {
            color: #fff;
        }

        .main-content-visible p,
        .main-content-visible span,
        .main-content-visible td,
        .main-content-visible th,
        .main-content-visible div,
        .main-content-visible li {
            color: #232e3c;
        }

        .dark .main-content-visible p,
        .dark .main-content-visible span,
        .dark .main-content-visible td,
        .dark .main-content-visible th,
        .dark .main-content-visible div,
        .dark .main-content-visible li {
            color: #f1f5f9;
        }
            ...
            <header class="header-gradient flex items-center justify-between border-b">
                <div class="flex items-center">
                    ...
                    <div class="relative ml-3 hidden md:block">
                        ...
                        <input type="text" id="manufacturerUniversalSearch"
                            class="w-80 py-2 pl-10 pr-4 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm"
                            placeholder="Search dashboard, stats, activities...">
                    </div>
                </div>
                <div class="flex items-center pr-4 space-x-3">
                    ...
                    <div class="relative">
                        <button
                            ...
                            <span class="mr-2 text-gray-700 font-medium text-sm">{{ Auth::user()->name ?? 'Admin User'
                                }}</span>
                    ...
                    <div class="flex items-center space-x-2">
                        <button type="button"
                            class="p-2 text-gray-500 hover:text-indigo-600 hover:bg-indigo-50 rounded-full transition-colors"
                            title="Edit Profile" x-data x-on:click="$dispatch('open-modal', 'profile-editor-modal')">
                    ...
                    <div class="grid grid-cols-1 gap-4 mt-4 lg:grid-cols-3 h-64">
                        <div class="card-gradient p-4 rounded-xl overflow-hidden lg:col-span-3">
                            ...
                            <div class="space-y-2 h-48 overflow-y-auto">
                                ...
                                <div
                                    class="flex items-start p-3 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow">
                                    <div
                                        class="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-full {{ $activity['color'] ?? 'bg-blue-100' }} bg-opacity-10">
                                        <i
                                            class="fas {{ $activity['icon'] ?? 'fa-info' }} {{ $activity['color'] ?? 'text-blue-600' }} text-sm"></i>
                                    </div>
                                    <div class="ml-3 flex-1">
                                        <p class="text-xs font-medium text-gray-800">{{ $activity['description'] ?? 'No
                                            activity' }}</p>
                    ...
                    <div class="mt-8">
                        ...
                        <div class="bg-white rounded-lg shadow p-6">
                            <div class="flex justify-between items-center mb-4">
                                ...
                                <a href="{{ route('manufacturer.work-orders.index') }}"
                                    class="text-blue-600 hover:underline">View All</a>
                            </div>
                                ...
                                <table class="min-w-full text-sm">
                                    ...
                                    <tbody>
                                        ...
                                        <tr class="hover:bg-gray-50 cursor-pointer"
                                            onclick="window.location='{{ route('manufacturer.production.show', ['production' => $order->id]) }}'">
                                            <td>{{ $order->id }}</td>
                                            ...
                                            <td>
                                                <div class="w-24 bg-gray-200 rounded-full h-3">
                                                    ...
                                                    @else bg-green-500
                                                    @endif" style="width: {{ $order->progress ?? 0 }}%">
                                                    </div>
                                                </div>
                                                <span class="text-xs text-gray-600 ml-1">{{ $order->progress ?? 0
                                                    }}%</span>
                                            </td>
                                            <td>
                                                <a href="{{ route('manufacturer.production.show', ['production' => $order->id]) }}"
                                        ...
                                        @empty
                                        <tr>
                                            <td colspan="7" class="text-center text-gray-400 py-4">No work orders found.
                                            </td>
                                        </tr>
                                        @endforelse
                                    </tbody>
                        ...
                        <div class="flex flex-col md:flex-row gap-4 mt-6">
                            <a href="{{ route('manufacturer.production.create') }}"
                                class="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2">
                                <i class="fas fa-plus text-blue-600 bg-white rounded-full p-1"></i> New Work Order
                            </a>
                            <a href="{{ route('manufacturer.bom.index') }}"
                                class="flex-1 bg-green-600 hover:bg-green-700 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2">
                                <i class="fas fa-cogs text-teal-600 bg-white rounded-full p-1"></i> Bill of Materials
                            </a>
                            <a href="{{ route('manufacturer.quality.index') }}"
                                class="flex-1 bg-yellow-500 hover:bg-yellow-600 text-white font-semibold py-3 rounded-lg flex items-center justify-center gap-2">
                                <i class="fas fa-clipboard-check text-amber-600 bg-white rounded-full p-1"></i> Quality
                                Checks
                            </a>
                        </div>
                    </div>
