...
<script>
    window.location = "{{ route('supplier.chat.index') }}";
</script>
@endif
<!DOCTYPE html>
<html lang="en">
            ...
            <div class="flex flex-col h-full">
                ...
                <nav class="flex-1 px-4 py-2 space-y-1">
                    <a href="{{ route('supplier.dashboard') }}"
                        class="nav-link flex items-center px-3 py-2 text-gray-300 hover:bg-green-700 hover:text-white rounded-xl">
                        <i class="fas fa-home w-5"></i>
                        <span class="ml-2 text-sm">Home</span>
                    </a>
                    <a href="{{ route('supplier.supply-requests.index') }}"
                        class="nav-link flex items-center px-3 py-2 text-gray-300 hover:bg-green-700 hover:text-white rounded-xl">
                    ...
                    <a href="{{ route('supplier.supplied-items.index') }}" class="nav-link flex items-center px-3 py-2 {{ request()->routeIs('supplier.supplied-items.index') || request()->routeIs('supplier.supplied-items.show') ? 'text-white bg-gradient-to-r from-green-600 to-green-700 shadow-lg' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }} rounded-xl">
                        ...
                        <span class="ml-2 text-sm">Supplied Items</span>
                    </a>
                    <a href="{{ route('supplier.analytics.index') }}"
                        class="nav-link flex items-center px-3 py-2 text-gray-300 hover:bg-green-700 hover:text-white rounded-xl">
                        <i class="fas fa-chart-bar w-5"></i>
                        <span class="ml-2 text-sm">Analytics</span>
                    </a>
                    <a href="{{ route('supplier.chat.index') }}"
                        class="nav-link flex items-center px-3 py-2 text-white bg-gradient-to-r from-green-600 to-green-700 rounded-xl shadow-lg">
                        <i class="fas fa-comments w-5"></i>
                        <span class="ml-2 text-sm">Chat</span>
                    </a>
                    <a href="{{ route('supplier.reports.index') }}"
                        class="nav-link flex items-center px-3 py-2 text-gray-300 hover:bg-green-700 hover:text-white rounded-xl">
                ...
                <div class="flex items-center pr-4 space-x-3">
                    ...
                    <div class="relative">
                        <button
                            ...
                            <span class="mr-2 text-gray-700 font-medium text-sm">{{ Auth::user()->name ?? 'Supplier
                                User' }}</span>
            ...
            <main class="flex-1 p-4">
                ...
                <div class="flex gap-6">
                    ...
                    <div class="w-80 card-gradient rounded-xl flex flex-col">
                        ...
                        <div class="flex-1 overflow-y-auto p-4 contacts-scroll"
                            ...
                            <div class="mb-6">
                                ...
                                <a href="{{ route('supplier.chat.show', ['contactId' => $manufacturer->id]) }}"
                                    class="contact-item flex items-center p-4 rounded-xl hover:bg-green-100 dark:hover:bg-green-900/20 transition"
                                    data-contact-id="{{ $manufacturer->id }}" data-contact-name="{{ $manufacturer->name }}">
                                    <div class="relative">
                                        <img src="{{ $manufacturer->profile_picture ? Storage::disk('public')->url($manufacturer->profile_picture) : asset('images/manufacturer.png') }}"
                                            alt="{{ $manufacturer->name }}"
                                            class="w-12 h-12 rounded-full border-2 border-purple-200">
                                        <span
                                            class="online-indicator absolute -bottom-1 -right-1 {{ $manufacturer->isOnline() ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                    </div>
                            ...
                            <div class="mb-6">
                                ...
                                <a href="{{ route('supplier.chat.show', ['contactId' => $admin->id]) }}"
                                    class="contact-item flex items-center p-4 rounded-xl hover:bg-green-100 dark:hover:bg-green-900/20 transition"
                                    ...
                                    <div class="relative">
                                        <img src="{{ asset('images/default-avatar.svg') }}" alt="{{ $admin->name }}"
                                            class="w-12 h-12 rounded-full border-2 border-purple-200">
                                        <span
                                            class="online-indicator absolute -bottom-1 -right-1 {{ $admin->isOnline() ? 'bg-green-500' : 'bg-gray-400' }}"></span>
                                    </div>
                                    <div class="ml-4 flex-1">
                                        <h5 class="text-sm font-medium text-gray-900 dark:text-white">{{ $admin->name }}
                                        </h5>
                    ...
                    <div class="flex-1 card-gradient rounded-xl flex flex-col min-h-0 bg-white overflow-hidden">
                        <div class="bg-gradient-to-r from-[#10B981] to-[#059669] p-6">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <p class="text-sm text-white dark:text-gray-400">{{ ucfirst($contact->role) }}
                                        <span
                                            class="ml-2 text-xs font-bold {{ $contact->isOnline() ? 'text-green-400' : 'text-gray-300' }}">
                        ...
                        <div class="p-6">
                            ...
                            <div class="flex-1 overflow-y-auto p-2 space-y-4 h-[300px]">
                                ...
                                : ($message->sender->profile_picture ? asset('storage/' . $message->sender->profile_picture) : asset('images/manufacturer.png'));
                                @endphp
                                <div class="flex {{ $isOwnMessage ? 'justify-end' : 'justify-start' }}">
                                    <div
                                        class="flex {{ $isOwnMessage ? 'flex-row-reverse' : 'flex-row' }} items-end max-w-xs lg:max-w-md">
                                        <img src="{{ $avatar }}" alt="{{ $senderName }}"
                                            class="w-8 h-8 rounded-full flex-shrink-0 border-2 border-green-200">
                                        <div class="message-bubble {{ $isOwnMessage ? 'own mr-3' : 'other ml-3' }}">
                                            <p class="text-sm">{{ $message->content }}</p>
                                            <p
                                                class="text-xs {{ $isOwnMessage ? 'text-green-100' : 'text-gray-500 dark:text-gray-400' }} mt-1">
